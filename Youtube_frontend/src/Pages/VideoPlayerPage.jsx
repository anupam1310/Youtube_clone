...
function VideoPlayerPage() {
  ...
  useEffect(() => {
    async function fetchVideo() {
      try {
        const response = await axios.get(`http://localhost:4050/api/video/${id}`);
  ...
  const handleLike = async () => {
    ...
    try {
      await axios.post(`http://localhost:4050/api/video/${id}/like`, {}, {
  ...
  const handleDislike = async () => {
    ...
    try {
      await axios.post(`http://localhost:4050/api/video/${id}/dislike`, {}, {
        headers: { Authorization: `BEARER ${token}` }
      });
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      setVideoData({ ...videoData, comments: [...videoData.comments, response.data] });
      setCommentText("");
    } catch (error) {}
  };
