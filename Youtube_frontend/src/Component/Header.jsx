...
// Header component with logo, search bar, and user authentication buttons
function Header() {
  ...
  // verifying token in local storage and checking user is logged in and if channel is created
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        const response = await axios.get("http://localhost:4050/api/verify", {
          headers: {
        ...
        if (response.data.user) {
          ...
          const channelResponse = await axios.get(
            `http://localhost:4050/api/channel/user/${userId}`,
            {
              headers: {
                Authorization: `BEARER ${token}`,
              },
    ...
    <header className="flex items-center justify-between px-4 py-2 bg-white shadow-md">
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={() => navigate("/")} className="hidden md:inline-flex">
          <img src={youtube_logo} alt="YouTube Logo" className="h-10 w-auto" />
          <span className="hidden md:inline  text-2xl text-gray-700">
            Youtube
          </span>
        </button>
      </div>
      ...
      <div className="flex items-center flex-1 max-w-xl mx-8">
        ...
        />
        <button
          className="bg-gray-100 px-4 py-2 border border-gray-300 rounded-r-full hover:bg-gray-200"
        ...
        {!isLoggedIn ? (
          ...
          <div className="flex items-center gap-2 md:gap-4">
            {!isChannelCreated && isLoggedIn ? (
              <button
                className="px-2 py-1 text-sm md:px-4 md:py-2 md:text-base bg-gray-100 rounded hover:bg-gray-200"
